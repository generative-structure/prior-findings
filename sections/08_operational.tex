...
detection in the two history groups, and no false positives.}}}
\end{center}
...
no false positives for the target condition, or its false-positive process is
separately modeled. False positives do not cancel in the ratio, even when
small; treating them as negligible is an approximation whose adequacy depends
on their size relative to the finding prevalence. The third is the
load-bearing one,
...
reference ratio $R_{\mathrm{ref}} = q^{\mathrm{ref}}_1/q^{\mathrm{ref}}_0$
equals $\Rlat$ times $s_1/s_0$ and the cancellation fails. Exact equality is
not needed for a usable result. If the organization can defend $\ell \le
s_1/s_0 \le u$, then
%
\begin{equation}\label{eq:refrange}
  \Rlat \;\in\; \bigl[\, R_{\mathrm{ref}}/u,\; R_{\mathrm{ref}}/\ell \,\bigr] ,
\end{equation}
%
and intersecting it with the record-based set~\eqref{eq:sharp} gives valid
combined bounds. The absolute reference finding rates can tighten them
further, because they also imply $r_f \ge q^{\mathrm{ref}}_f$. A reference
ratio of 2 with sensitivities defended to within ten percent of each other
($\ell = 0.9$, $u = 1.1$) places $\Rlat$ between 1.82 and 2.22. Reference-sample size should be planned from the desired
precision and the expected condition or finding rates in the two history
groups.

\subsection{High-sensitivity reference procedures}

A second anchor comes from a procedure whose sensitivity $s$ for a narrowly
defined condition is known, or can be bounded from below. Applied to a whole
subpopulation with no false positives, its finding rate $q^{\mathrm{ref}}$
determines that subpopulation's condition rate in one of three grades:
%
\begin{itemize}
\item perfect sensitivity, $s = 1$: $r = q^{\mathrm{ref}}$;
\item known sensitivity, $0 < s < 1$: $r = q^{\mathrm{ref}}/s$;
\item defended lower bound, $s \ge s_L$:
  $r \in [\,q^{\mathrm{ref}},\; \min(1,\, q^{\mathrm{ref}}/s_L)\,]$.
\end{itemize}
%
Near-perfect sensitivity is the third grade with $s_L$ close to one: at $s_L =
0.95$, $r$ is at most about 5.3 percent above $q^{\mathrm{ref}}$. The targeted
procedure's finding rate $q$ in the same group then gives its detection
probability, $d = q/r$, exactly in the first two grades and within
$[\,q/\min(1, q^{\mathrm{ref}}/s_L),\; q/q^{\mathrm{ref}}\,]$ in the third.
Anchors in both history groups yield $d_0$, $d_1$ and $M$, or bounds on
them (Proposition~\ref{prop:ref}).
...
sampling or coverage uncertainty, and it anchors the underlying rate only to
the extent that the procedure's sensitivity for the target condition is known
or bounded. Where auditors speak of ``certainty
...
transaction for an exact duplicate payment can have very high sensitivity
for that narrowly defined condition; testing every file for a
judgment-dependent compliance failure need not, and the anchor is then only as
...
re-performing the same sample with the same method do not.
...
creates, or tightens bounds; absolute condition prevalence requires
