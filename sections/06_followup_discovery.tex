...
the regulation is right to require the visit. It is not evidence that a
condition remains; the follow-up procedures generate that evidence, and the
...
Prior findings may carry genuine information about it: some entities are
...
nothing, using the same random draws, so the zeros are exact and the
opportunity-set artifact of Section~\ref{sec:fac} cancels. New findings are
