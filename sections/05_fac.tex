\section{Clearinghouse evidence: recurrence, new findings, and mixed cases}\label{sec:fac}
...
recurrence, findings not classified as repeats, and mixed cases.
...
Write $J_t = 1$ when the program is audited as a major program in year $t$.
Every Sample~A quantity is conditional on $J_{t-1} = J_t = 1$: the comparisons
are $P(F_t = 1 \mid F_{t-1} = f,\, J_{t-1} = J_t = 1)$ for $f = 0, 1$, and the
empirical statements below apply to that selected population. Selection into
it depends on the finding record (Section~\ref{sec:regime}), so the
conditioning defines the population described; it does not make selection
ignorable for any wider population.
...
(Appendix~\ref{app:emp}). A finding is \emph{new} when the auditor did not
classify it as a repeat. ``New'' is shorthand for that classification: it does
not establish that the condition was never recorded before or that it arose
this year.
...
recurrence, and additional findings not classified as repeats. Table~\ref{tab:fourstate}
...
\end{equation}
%
The unrounded values satisfy the identity exactly; the four-decimal values
shown differ in the last digit by rounding. A program-year following a
prior-year finding records about one more finding than one following a clean
year, of which about seven tenths of a finding is a repeat and three tenths is
new.
...
\subsection{A recurrence share depends on how mixed outcomes are allocated}

The obvious way to compress Table~\ref{tab:fourstate} into one number is to
ask what share of the prior-finding association is recurrence. Every mixed
period has to be assigned to one side first; mixed periods are
\nMixedShare{} percent of the program-years with a finding in the
prior-finding group. Assign none of them to recurrence and the share is
\nZetaLo{} percent. Assign all of them and it is \nZetaHi{} percent. The
summary moves \nZetaSwing{} percentage points, and nothing in the underlying
observations changes (Fig.~\ref{fig:zeta}). A two-category recurrence share
is not uniquely defined until the treatment of mixed outcomes is specified,
and it is not a causal attribution. For a fixed allocation fraction $\zeta$ it
is a well-defined descriptive function of observable quantities. The general
lesson: when outcome classes overlap, report mutually exclusive states
...
finding this year, independent of each other and of anything before. A program with neither
