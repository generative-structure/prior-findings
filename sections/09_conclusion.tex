...
observed ratio is below one, the underlying association is negative; in data,
that conclusion requires an upper confidence limit for the observed ratio
below one.
...
Follow-up uses history to revisit known conditions, and targeted follow-up
can discover a new condition. Identifying the relationship between history
and underlying discovery risk requires observation whose measurement rule is
not itself distorted by the history being evaluated, or a defensible model of
that distortion. In
Clearinghouse data, among programs audited as major programs in both years,
prior findings predict recurrence and materially more findings not classified
as repeats, and the record does not identify why.
...
sensitivity is common to both history groups or its ratio is bounded; a
reference procedure of known or bounded sensitivity that fixes or bounds the
condition rate; reference adjudication accurate
...
ceiling $\Mbar$, under which the underlying ratio lies in
$[\max\{q_1, \Robs/\Mbar\}, \Robs]$.
Each is an addition to audit design, not a statistical method applied to the
record.

Five boundaries travel with these conclusions. Repeat findings are not
artifacts; a repeat finding in these data is a real reported deficiency the
auditor identified as a recurrence. A ``new'' finding is one the auditor did
not classify as a repeat, a statement about that classification and not about
when the problem arose or whether it was recorded before. The identity $q =
rd$ and every bound built on it assume no false positives, or a modeled
restriction on them. The predictive results are
